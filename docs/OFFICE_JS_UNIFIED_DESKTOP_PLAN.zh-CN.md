...
- `tools/unified-smoke-test.ps1` 已验证 TypeScript 类型检查、40 个工具完整模拟 dispatch、四档 Word API/WordApiDesktop capability matrix、Vite production bundle、HTTPS/loopback 发布地址拒绝、`WordOllama.JS` Add-in ZIP/生产 manifest、.NET Bridge 构建、安全默认值和 Skill 复制；脚本会检查每个外部命令的退出码。Microsoft manifest 在线校验通过，目标平台包含 Windows、Mac、Web 和 iPad。
- 任务窗格已加入真实 Word “40 项宿主金样本”运行器：在明确确认后为一次性文档建立固定夹具，按宿主 capability 过滤，逐项隔离失败并导出含 Word 版本/平台的 JSON 报告；自动测试覆盖 40 项唯一性、能力过滤、失败隔离、夹具阻塞和中英文内置样式映射。
- Windows Word 16.0.20228.20110（zh-CN）已通过真正的 `WordOllama.JS` 标签完成当时版本的 36/36 宿主金样本，报告归档于 `docs/evidence/windows-word-16.0.20228.20110-golden-2026-07-29.json`；新增的 2 个表格工具、精确修订工具和安全精确选中工具尚待重新完成 40/40 验收。首次运行暴露 WebView2 不支持 `window.prompt()`，现已用窗格内 HTML 对话框替代，并同时移除金样本对 `window.confirm()` 的依赖。
...
- `packaging/sign-bridge-release.ps1` 已加入 Windows Authenticode、macOS codesign/notarytool 和签后归档流程；Windows 会优先使用 PATH 中的 `signtool.exe`，否则自动发现最新 Windows 10 SDK x64 工具。Windows 首发正式版允许非 CA、仅限 Code Signing EKU 的产品自签名证书，要求用户显式建立当前用户信任并强制 RFC 3161 时间戳；发布证据和更新器固定 Subject、证书指纹与公钥哈希。只有无时间戳签名仍被限制在显式 smoke/test 回归。macOS Developer ID 入口强制 `Developer ID Application` 身份与 notary profile，并把 Apple 返回的 submission ID、Accepted 状态、完整公证日志哈希、签名 Authority/Team ID、Hardened Runtime、安全时间戳及签名后 ZIP SHA-256 固化为独立证据；首发本地自签名模式则明确记录显式用户信任且不伪造 Apple 公证。终审要求同一版本/架构/ZIP 的证据并再次执行对应平台签名验收。正式跨机器签名仍需在对应发布机执行。
- 设置页已接入签名平台安装器的一键更新：只有生产索引中带当前 runtime、SHA-256、精确大小和发布者的 EXE/PKG 才显示安装入口。Bridge 每次操作重新获取索引，限制下载为 512 MB，下载过程中同步计算哈希；索引声明的发布者还必须与上一版签名安装器写入 Bridge 配置的固定发布者一致。Windows 使用 Authenticode 检查当前用户信任链、RFC 3161 时间戳、精确 Subject、证书指纹和公钥哈希，支持用户显式信任的产品自签名发布证书；macOS 按 Developer ID 或明确的本地自签名模式校验固定 Installer Authority，验证后才交给系统安装器。任何异常都会清理临时/最终下载文件。自动测试覆盖正确下载、错误哈希、缺失/不匹配发布者、真实平台对未签名文件的拒绝，以及端点未配对/未配置失败关闭。
...
- `WordOllama.WindowsInstaller` 与 `packaging/package-windows-installer.ps1` 已补齐 Windows 用户安装载体：生成内嵌签名 Bridge ZIP 和独立 SHA-256 元数据的 .NET 8 自包含单文件 WinExe，安装到 `%LOCALAPPDATA%`，支持幂等重装、版本指针、Apps & Features 当前用户卸载登记、隐藏 Startup、自有进程停止及完整卸载；PFX 未配置时不反复启动，Credential Manager 配置完成后自动激活。生产 EXE 强制有效 Authenticode、精确 publisher 和 RFC 3161 时间戳；首发自签名发布证书必须是非 CA、仅限 Code Signing EKU，并由用户显式加入当前用户信任。流程生成与 Bridge ZIP 绑定的安装器及发布者证书证据，终审重新验证 EXE 哈希、Subject、证书/公钥/时间戳指纹并固化为 `desktop-bridge-installer`。自动回归已实际构建并在隔离目录两次安装和卸载 unsigned smoke 安装器；正式签名证据仍须发布证书环境生成。
...
仍需在真实发布环境完成的门槛：macOS 目标 Word 版本的 40 项宿主金样本、长文档性能和修订运行器，以及 Windows/Mac 的真实双客户端共同编辑和目录 API capability 降级；正式 HTTPS 证书链与真实平台密钥库写入验收、Windows 签名、macOS Developer ID/notarization；真实复杂合同的 comparer v2 验证及类似原生修订的逐项接受/拒绝应用流程。Windows 当前 Word 版本已通过旧版 36 项工具、1,000/5,000 段单客户端长文档及修订 API；新增的 2 个表格工具、精确修订工具和安全精确选中工具仍需真机复验，这些剩余项未实测前仍不能宣称与 VSTO 完全等价。
