...
- [ ] 为每个 Ribbon 命令、任务窗格功能入口和设置导航项配置语义不同的仓库原生
  SVG 图标；统一使用当前 Word 蓝白视觉和一致的描边、画布、留白规范，不复用旧
  COM/XAML 位图，也不以同一个通用 AI 图标代替不同功能。
- [ ] 建立 SVG 图标清单与自动校验，确保 manifest/Ribbon 和 React UI 引用的图标
...
- [ ] `run_python`、`run_node` 继续保持关闭；仅限制工作目录并不是真沙箱，须实现
  Windows 受限令牌/AppContainer 与 macOS 对应隔离后端、进程树终止和默认断网。
