...
- [ ] 桌面同源模式改为 Bridge 托管页面自动建立 HttpOnly、SameSite 会话，普通用户
  不需要从日志复制随机配对码。
- [ ] 跨 origin 开发模式使用仅开发构建可见的诊断授权，不进入正式 Ribbon；相关
  状态、日志导出和连接测试统一放到“设置 > 进阶 > 诊断”。
- [ ] 防止任意本地网页伪造 `Origin` 后获取 Bridge 会话。
- [ ] 增加 CSRF token、自定义请求头、严格 CORS、禁止第三方 frame 和敏感 API
  Content-Type 校验；同源自动会话不能等同于无鉴权 API。
...
- [ ] Windows 安装器为当前用户安装并信任 WordOllama.JS 自签名发布证书，签署
  Bridge 和 EXE，更新器固定证书指纹/公钥并继续校验 SHA-256。
...
- [x] 产品决策：Ollama 完全作为外部依赖，WordOllama.JS 不安装、不更新、不配置、
  不删除 Ollama 或其模型。
- [ ] 检测 Ollama 是否已安装/运行；不可用时给出本地化安装指引、官方链接和重新
...
- [x] 产品决策：Agent 面板增加权限模式和“目标”。权限模式为“请求批准”、
  ...
  限制，并支持暂停、继续和停止。
- [ ] 接入可选 Search MCP，提供搜索结果、正文摘要和来源元数据；默认不开启，支持
  域名权限、调用次数和结果大小限制。
- [ ] 将现有直接 HTTPS 工具收敛为 `fetch_url`：限制 HTTPS、重定向、DNS 重绑定、
  私网地址、响应大小、MIME、超时和凭据，并提取可供模型使用的正文与来源信息。
- [ ] 新增每任务隔离工作区及 `read_file`、`write_file`、`run_python`、`run_node`
  工具；限制目录、执行时间、输出、进程树和默认网络访问。
- [ ] 实现三档权限：请求批准始终确认副作用，替我审批只自动允许隔离工作区内的
  低风险操作，完全访问权限才开放完整终端、外部文件和网络。完全访问权限启用时
  显示持续危险状态并至少进行会话级二次确认，不能由提示词或模型自行开启。
- [ ] 实现 Agent 目标的持久化、检查点、停止条件、循环上限和异常恢复；目标不能
  绕过权限确认或扩大文件/网络范围。
- [ ] 为本地临时代码工作区增加清理、取消、进程树终止、符号链接越界和恶意参数
  回归；为完整终端增加命令审计、敏感参数隐藏和 Windows/macOS 双平台回归。完成前
  继续推荐把稳定脚本封装为 Skill 或 MCP。
