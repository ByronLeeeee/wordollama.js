...
- [x] 记录证书指纹；普通升级复用仍有效且配置完整的本产品证书，仅在缺失、损坏、
  临近过期或显式安全维护时轮换，卸载时只删除本产品拥有的证书。
- [x] macOS PKG 安装显式的“完成安全设置”入口；用户再次确认后才调用当前用户
  Trust Settings，且不绕过 macOS 自己的授权提示，随后执行启动与双端健康检查。
- [ ] 为证书创建、信任、轮换和卸载添加目标平台实机回归。
  Windows 当前用户实机已于 2026-08-02 完成创建、普通升级复用、显式轮换和卸载
  清理；此项只等待干净 Windows 用户复验与 Apple Silicon Mac 上的同等证据。
...
- [x] 验证 Authenticode、自签名发布者固定值、普通升级证书复用、显式证书轮换和
  卸载清理；Windows 生命周期记录器及独立验证器已于 2026-08-02 对签名候选通过。
  文档明确说明
...
- [x] Agent 增加 30 分钟/200 轮硬预算、上下文压缩、计划动态更新和运行指标，避免所谓
  “无限迭代”导致上下文失控；OpenAI 兼容普通请求对 429/502/503/504 使用有界退避。
- [x] Agent 自动采集 Search MCP、Fetch、MCP 与向量检索结果中的外部 URL，在面板列出
  可点击来源；最终回答提示必须基于实际返回证据，不能编造引用。
- [x] 新增任务完成后“整理为 Skill”、`/make-skill`、Agent `create_skill` 工具和设置页
  AI Skill 制作器；生成内容结合需求、成功结果、工具和用户反馈，严格校验名称、
  YAML 前言、大小、重复名称和 Skills 根目录边界。
- [x] 模型详情增加能力体检，实际验证模型列表、普通对话、流式响应和原生工具调用；
  长文档段落读取只载入目标范围文本，语义地图优先按 Word 标题结构分段；法务引用
  正确区分“格式检查”和“来源核验”，交叉引用不再无条件返回已找到。
