...
if ($golden.schemaVersion -ne 1 -or $golden.passed -ne 40 -or $golden.failed -ne 0 -or
    $golden.unsupported -ne 0 -or $golden.blocked -ne 0 -or
    @($golden.results).Count -ne 40 -or
    @($golden.results | Where-Object { $_.status -ne "passed" }).Count -ne 0) {
    throw "Golden report does not prove all 40 Word tools passed."
}
