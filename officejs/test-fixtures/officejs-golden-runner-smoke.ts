...
if (names.length !== 36 || new Set(names).size !== 36) {
  throw new Error(`golden runner must contain 36 unique tools, got ${names.length}`);
}
...
if (report.results.length !== 36 || progressEvents !== 36) {
  throw new Error("golden runner did not report every tool");
}
if (report.supportedToolCount !== 35 || report.passed !== 34 || report.failed !== 1 ||
...
if (blockedReport.blocked !== 35 || blockedReport.unsupported !== 1 ||
...
console.log("Office.js golden runner smoke passed (36 tools, locale-safe styles, capability and failure isolation).");
