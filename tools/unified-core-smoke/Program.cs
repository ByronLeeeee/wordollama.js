...
try
{
    ...
    reviewSettings.Save(new ReviewSettingsUpdate(" concise legal style "));
    Assert(reviewSettings.Get().WritingProfile == "concise legal style",
        "writing profile is persisted by the Bridge");
    var reloadedReviewSettings = new ReviewSettingsStore(Path.Combine(settingsTestRoot, "review.json"));
    Assert(reloadedReviewSettings.Get().WritingProfile == "concise legal style",
        "writing profile reloads across Bridge restarts");
}
finally
{
